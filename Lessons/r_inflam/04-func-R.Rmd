...
That seems almost right: the original mean was about `r round(mean(inflam[, 4]), 2)`, so the lower bound from zero is now about `r -round(mean(inflam[, 4]), 2)`.
...
answer <- rescale(inflam[, 4])
min(answer)
max(answer)
plot(answer)
plot(inflam[, 4], answer)  # This hasn't been introduced yet, but it may be
...
answer <- rescale(inflam[, 4], lower = 2, upper = 5)
min(answer)
max(answer)
answer <- rescale(inflam[, 4], lower = -5, upper = -2)
min(answer)
max(answer)
```
